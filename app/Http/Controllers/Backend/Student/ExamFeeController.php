...
class ExamFeeController extends Controller
{
     ...
     public function ExamFeeClassData(Request $request){
        $year_id = $request->year_id;
         ...
         foreach ($allStudent as $key => $v) {
            ...
            $html[$key]['tdsource'] .='<a class="btn btn-sm btn-'.$color.'" title="PaySlip" target="_blanks" href="'.route("student.exam.fee.payslip").'?class_id='.$v->class_id.'&student_id='.$v->student_id.'&exam_types_id='.$request->exam_types_id.' ">Fee Slip</a>';
            $html[$key]['tdsource'] .= '</td>';
     ...
     public function ExamFeePayslip(Request $request){
        ...
        $class_id = $request->class_id;
        $data['exam_type'] = ExamType::where('id',$request->exam_types_id)->first()['name'];
